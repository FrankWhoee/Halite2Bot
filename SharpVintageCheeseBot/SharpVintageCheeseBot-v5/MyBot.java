...
public class MyBot {
    ...
    public static void main(final String[] args) {
        ...
        final GameMap gameMap = networking.initialize("SharpVintageCheeseBot-v5");
            ...
            if(cheese == false){
                ...
                for (final Ship ship : gameMap.getMyPlayer().getShips().values()) {
                    ...
                    for (final Planet planet : gameMap.getSortedPlanets(ship)) {
                        ...
                        if(ship.canDock(planet) && !planet.isFull() && planet.getHealth() > 500 && dangerousEnemyShips.size() == 0){
                            Log.log("Ship [" + ship.getId() +"] docking with planet [" + planet.getId() + "]");
                            moveList.add(new DockMove(ship,planet));
                            break;
                        }else if(gameMap.allPlanetsTaken() || ship.getDistanceTo(sortedEnemyShips.get(0)) < 25 && (ship.getId()  % 2 != 0)){
                            ...
                            if(ship.getId() % 2 == 0){
                                ...
                                ThrustMove attackMove = Navigation.planetSpiralMove(ship, sortedEnemyShips.get(0), gameMap);
                                    ...
                                    Log.log("Ship [" + ship.getId() +"] attacking other ships because shipId mod 2 is 0. Attacking ship [" + sortedEnemyShips.get(0).getId() + "]");
                                    break;

...
public class GameMap {
    ...
    public ArrayList<Ship> nearestVulnerableEnemyShips(Ship thisShip) {
        ...
        for(Ship ship: getAllShips()){
            if(!myShips.contains(ship) && ship.getDockingStatus() == Ship.DockingStatus.Undocked && thisShip.getDistanceTo(ship) > 40){
    ...
    public ArrayList<Position> getClosestHideouts(Ship ship){
        ArrayList<Position> sortedHideouts = new ArrayList<>();
        for(int i = 0; i < (int)getMyPlayer().getShips().size()/4; i++){
        sortedHideouts.add(new Position(5,5));
        sortedHideouts.add(new Position(getWidth() - 5,getHeight() - 5));
        sortedHideouts.add(new Position(getWidth() - 5, 5));
        sortedHideouts.add(new Position(5,getHeight() - 5));
        }
        ...
        int index;
        for(int k = 0; k < sortedHideouts.size() - 1; k++){
            index = k;
            for(int i = k + 1; i < sortedHideouts.size(); i++){
                if(sortedHideouts.get(index).getDistanceTo(ship) > sortedHideouts.get(i).getDistanceTo(ship)){
                    index = i;
                }
            }
            removed = sortedHideouts.get(k);
            sortedHideouts.set(k, sortedHideouts.get(index));
            sortedHideouts.set(index, removed);
        }
        
        return sortedHideouts;
...
public ArrayList<Ship> getSortedDangerousEnemyShips(Ship thisShip){
    ArrayList<Ship> sortedShips = new ArrayList<>();
    Ship removed;
    sortedShips = new ArrayList<>();
        
        for(Ship ship: getAllShips()){
            if(ship.getOwner() != getMyPlayerId() && ship.getDockingStatus() == Ship.DockingStatus.Undocked){
                sortedShips.add(ship);
            }
        }
        int index;
...
public ArrayList<Ship> getSortedNearbyDangerousEnemyShips(Ship thisShip){
    ArrayList<Ship> sortedShips = new ArrayList<>();
    Ship removed;
    sortedShips = new ArrayList<>();
        
        for(Ship ship: getAllShips()){
            if(ship.getOwner() != getMyPlayerId() && ship.getDockingStatus() == Ship.DockingStatus.Undocked && thisShip.getDistanceTo(ship) < 20){
                sortedShips.add(ship);
            }
        }
        int index;

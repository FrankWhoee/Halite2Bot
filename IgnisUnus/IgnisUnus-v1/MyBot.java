...
public class MyBot {
    ...
    public static void main(final String[] args) {
        ...
        final GameMap gameMap = networking.initialize("PostAurum-v7");
                ...
                for (final Ship ship : gameMap.getMyPlayer().getShips().values()) {
                    ...
                    if(dangerousEnemyShips.size() > 0 && ship.getDockingStatus() == Ship.DockingStatus.Undocked){
                       ...
                       for(Ship teamShip: gameMap.getMyPlayer().getShips().values()){
                           if(ship.getDistanceTo(teamShip) < 20 && teamShip.getDockingStatus() == Ship.DockingStatus.Undocked){
                               numOfSurroundingTeamShips++;
                           }
                       }
                       if(numOfSurroundingEnemyShips >= numOfSurroundingTeamShips){
                            if(teamShips.size() > 0 && ship.getDistanceTo(teamShips.get(0)) < 28){
                                Position retreatPoint = ship.getClosestPoint(teamShips.get(0));

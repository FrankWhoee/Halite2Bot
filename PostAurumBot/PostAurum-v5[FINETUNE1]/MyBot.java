...
public class MyBot {
    ...
    public static void main(final String[] args) {
        ...
        final GameMap gameMap = networking.initialize("PostAurum-v5");
        ...
        for (;;) {
            ...
            if(cheese){
                for (final Ship ship : gameMap.getMyPlayer().getShips().values()) {
                    ...
                    if(dangerousEnemyShips.size() > 0){
                       ...
                       for(Ship enemyShip: dangerousEnemyShips){
                           if(ship.getDistanceTo(enemyShip) < 15){
                               numOfSurroundingEnemyShips++;
                           }
                       }
                       int numOfSurroundingTeamShips = 0;
                       for(Ship teamShips: gameMap.getMyPlayer().getShips().values()){
                           if(ship.getDistanceTo(teamShips) < 15 && teamShips.getDockingStatus() == Ship.DockingStatus.Undocked){
                ...
                for (final Ship ship : gameMap.getMyPlayer().getShips().values()) {
                    ...
                    //Attack
                    if(ship.getDistanceTo(enemyShips.get(0)) < 35 || gameMap.isAllPlanetsTaken() || ship.getId() == earlyAggressor.getId()){

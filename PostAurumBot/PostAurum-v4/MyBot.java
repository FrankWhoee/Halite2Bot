...
public class MyBot {
    ...
    public static void main(final String[] args) {
        ...
        final GameMap gameMap = networking.initialize("PostAurum-v4");
        ...
        Ship earlyAggressor = gameMap.getMyPlayer().getShip(0);
            ...
            if(cheese){
                ...
                for (final Ship ship : gameMap.getMyPlayer().getShips().values()) {
                    ...
                    if(dangerousEnemyShips.size() > 0){
                       int numOfSurroundingEnemyShips = 0;
                       for(Ship enemyShip: dangerousEnemyShips){
                           if(ship.getDistanceTo(enemyShip) < 25){
                               numOfSurroundingEnemyShips++;
                           }
                       }
                       int numOfSurroundingTeamShips = 0;
                       for(Ship teamShips: gameMap.getMyPlayer().getShips().values()){
                           if(ship.getDistanceTo(teamShips) < 25 && teamShips.getDockingStatus() == Ship.DockingStatus.Undocked){
                               numOfSurroundingTeamShips++;
                           }
                       }
                       if(numOfSurroundingEnemyShips >= numOfSurroundingTeamShips){
                    ...
                    if(ship.getDistanceTo(enemyShips.get(0)) < 35 || gameMap.isAllPlanetsTaken() || ship == earlyAggressor){
                    ...
                    for (final Planet planet : gameMap.getSortedPlanets(ship)) {
                        ...
                        if(ship.canDock(planet) && !planet.isFull() && !cheesed){
                            ...
                            break;
                        }else if(!planet.isOwned() || (planet.isOwned() && planet.getOwner() == gameMap.getMyPlayerId()) && !planet.isFull() && !cheesed){

        ...
        for (;;) {
            ...
            if(cheese){
                ...
                for (final Ship ship : gameMap.getMyPlayer().getShips().values()) {
                    ...
                    ArrayList<Ship> teamUndocked = gameMap.getUndockedTeamShips(ship);
                    if(dangerousEnemyShips.size() > 0 && ship.getDockingStatus() == Ship.DockingStatus.Undocked){
                       ...
                       if(numOfSurroundingEnemyShips >= numOfSurroundingTeamShips){
                           ...
                           ThrustMove retreatMove;
                           if(ship.getDistanceTo(teamUndocked.get(0)) < 14 && teamUndocked.get(0) != null){
                               retreatPoint = teamUndocked.get(0);
                               Log.log("retreat point = (" + retreatPoint.getXPos() + "," + retreatPoint.getYPos() + ")");
                               Log.log("retreating to ship " + teamUndocked.get(0).getId());
                               Log.log("" + teamUndocked);
                               retreatMove = Navigation.navigateShipTowardsTarget(gameMap, ship, retreatPoint, Constants.MAX_SPEED, true, Constants.MAX_NAVIGATION_CORRECTIONS, Math.PI/180);
                           }else{
                               retreatPoint = dangerousEnemyShips.get(0);
                               retreatMove = Navigation.retreatMove(ship, gameMap, dangerousEnemyShips.get(0), Constants.MAX_SPEED, Math.PI/180);
                           }

...
public class Navigation {
    ...
    public static ThrustMove retreatMove(Ship ship, GameMap gameMap, ArrayList<Ship> enemyShips, int maxThrust, double angularStepRad){
        
        final double radian = Math.PI * 2;
        double angleRad = ((ship.orientTowardsInRad(enemyShips.get(0))%radian + radian/2)+radian)%radian;
        
        Position targetPos = getNewTarget(ship, angleRad, 0, maxThrust);
        double distanceNegative = ship.getDistanceTo(targetPos);
        double distancePositive = ship.getDistanceTo(targetPos);
        
        for(double divergence = 0; divergence < Math.PI * 2; divergence += angularStepRad){
            Position newTarget = getNewTarget(ship,angleRad, divergence, maxThrust);
            distanceNegative = ship.getDistanceTo(newTarget);
            if(!(newTarget.getXPos() < 0 || newTarget.getXPos() > gameMap.getWidth() || newTarget.getYPos() < 0 || newTarget.getYPos() > gameMap.getHeight())){
                if(gameMap.objectsBetween(ship, newTarget).isEmpty()){

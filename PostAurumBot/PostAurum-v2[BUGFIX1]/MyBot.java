...
public class MyBot {
    ...
    public static void main(final String[] args) {
        ...
        final GameMap gameMap = networking.initialize("PostAurum-v2");
        ...
        for (;;) {
            ...
            if(cheese){
                ...
                for (final Ship ship : gameMap.getMyPlayer().getShips().values()) {
                    ArrayList<Ship> enemyShips = gameMap.getSortedEnemyShips(ship);
                    ArrayList<Ship> dangerousEnemyShips = enemyShips;
                    for(int index = 0; index < dangerousEnemyShips.size(); index++){
                        if(dangerousEnemyShips.get(index).getDockingStatus() == Ship.DockingStatus.Docked){
                            dangerousEnemyShips.remove(index);
                        }
                    }

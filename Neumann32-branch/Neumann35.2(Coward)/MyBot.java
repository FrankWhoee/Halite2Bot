    ...
    public static void main(final String[] args) {
        ...
        for (;;) {
            ...
            for (final Ship ship : gameMap.getMyPlayer().getShips().values()) {
                ...
                if(turnNum > 135 && gameMap.getMyPlayer().getShips().size() < 20){
                    ThrustMove runMove;
                        runMove = Navigation.navigateShipTowardsTarget(gameMap, ship, new Position(ship.getId(), 0), Constants.MAX_SPEED, true, Constants.MAX_NAVIGATION_CORRECTIONS, Math.PI/180.0);
                ...
                if(sortedDangerousEnemyShips.size() > 0){
                    if(ship.getDistanceTo(sortedDangerousEnemyShips.get(0)) < 42 && (ship.getDockingStatus() == Ship.DockingStatus.Undocked) && ship.getDistanceTo(sortedDangerousEnemyShips.get(0)) > 4.5){
                        ...
                        attackMove = Navigation.navigateShipTowardsTarget(gameMap, ship,sortedDangerousEnemyShips.get(0), 5, true, Constants.MAX_NAVIGATION_CORRECTIONS, Math.PI/180.0);
                        //queue up attack move and stop considering planets
                        if (attackMove != null) {
                ...
                */
                ...
                if(gameMap.getPlanet(ship.getDockedPlanet()).getHealth() < 500 && (ship.getDockingStatus() == Ship.DockingStatus.Docked)){
                      moveList.add(new UndockMove(ship));
                      continue;
                }
